...
function renderQuizQuestion() {
  ...
  const allWords = getAllWords();
  const wrongOptions = shuffle(allWords.filter(w => w.emoji !== q.emoji)).slice(0, 3).map(w => w.emoji);
  ...
  container.innerHTML = `
    <div class="quiz-card">
      ...
      <div class="quiz-options">
        ${options.map((opt, idx) => `
          <button class="quiz-option" data-eid="${opt}" data-hebrew="${q.hebrew}" data-translit="${q.translit}"
